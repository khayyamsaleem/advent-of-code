...
fn parse_rucksacks_compartments(input: &str) -> impl Iterator<Item=(&str,&str)>{
    parse_rucksacks(input)
        .map(|rucksack| rucksack.split_at(rucksack.chars().count()/2))
}
...
fn total_priority_for_elf_group_badges<'a>(
    rucksacks: impl Iterator<Item=&'a str>
) -> u32 {
    rucksacks.tuples::<(_,_,_)>().fold(0, |acc, (fst,snd,thd)|{
        let fst_items = fst.chars().collect::<HashSet<char>>();
        let snd_items = snd.chars().collect::<HashSet<char>>();
...
";
    ...
    fn test_total_priority() {
        assert_eq!(total_priority(parse_rucksacks_compartments(TEST_INPUT)), 157)
    }
