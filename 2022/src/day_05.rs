...
fn get_stacks_and_operations(input: &str) -> (Vec<Stack>, impl Iterator<Item=Operation> + '_) {
    ...
    let num_stacks: usize = raw_lst_row.trim().rsplit(&" ".repeat(CRATE_WIDTH)).next().unwrap().parse().unwrap();
    (
        raw_stacks_iter.fold::<Vec<Stack>,_>(vec![vec![]; num_stacks], |mut acc, raw_crates| {
            raw_crates.chars().chunks(CRATE_WIDTH + CRATE_GAP).into_iter().enumerate().for_each(|(i,mut c)| {
                if c.next().unwrap() == ' ' {return}; // check if its whitespace
...
fn rearrange_crates_cratemover_9001(stacks: &mut Vec<Stack>, operations: impl Iterator<Item=Operation>) {
    operations.for_each(|op| {
...
";
    ...
    fn test_get_stacks_and_operations() {
        ...
        assert_eq!(
            ...
            vec![
                Operation{n_crates: 1, source_stack: 2, target_stack: 1},
                Operation{n_crates: 3, source_stack: 1, target_stack: 3},
                Operation{n_crates: 2, source_stack: 2, target_stack: 1},
                Operation{n_crates: 1, source_stack: 1, target_stack: 2},
            ]
        );
    }
