...
fn create_tree_from_input(input: String) -> Graph<String, u64> {
    input.trim()
        ...
        .fold(Graph::new(), |mut acc, cur| {
            ...
            iter.skip(2).intersperse(" ").collect::<String>().split(", ").for_each(|bag_exp| {
                let mut bag_exp_iter = bag_exp.split(" ");
                let num_bags= bag_exp_iter.next().unwrap().parse::<u64>().unwrap_or_default(); // weight, zero if "no"
                if num_bags == 0 { return };
                let nested_bag_name = bag_exp_iter.next().unwrap().to_owned() + " " + bag_exp_iter.next().unwrap(); // shade, color
...
fn count_bag_capacity(bag_name: &str, g: &Graph<String, u64>, init_weight: u64) -> u64 {
    let node_to_count_for = g.node_indices().find(|n| g[*n] == bag_name).unwrap_or_else(|| panic!("No such node: {}", bag_name));
    let neighbors = g.neighbors(node_to_count_for);
    let sum: u64 = neighbors.map(|n| {
...
pub async fn solve() -> Result<(), Error> {
    ...
    println!("Day 07 Part 1: {:?}", g.node_indices().filter(|n| has_shiny_gold_bags(&g, n) && g[*n] != BAG_TO_FIND).count());
    println!("Day 07 Part 2: {:?}", count_bag_capacity(BAG_TO_FIND, &g, 1) - 1);
    Ok(())
}
...
mod tests {
    ...
    async fn test_create_tree() -> Result<(), reqwest::Error>{
        let _m = mockito::mock("GET", "/2020/day/7/input")
            .with_body("light red bags contain 1 bright white bag, 2 muted yellow bags.
