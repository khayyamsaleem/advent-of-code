...
fn is_valid_passport_basic(
    ...
    let only_required = passport.keys().filter(|k| !optional_fields.contains_key(k.as_str()));
...
fn is_valid_passport_advanced(
    ...
    let only_required : Vec<&String> = passport.keys().filter(|k| !optional_fields.contains_key(k.as_str())).collect();

    if only_required.len() != required_fields.len() { return false };

    only_required.iter().all(|k| {
        required_fields.get(k.as_str()).unwrap()(passport.get(k.as_str()).unwrap())
    })
}

fn get_field_rules() -> (HashMap<&'static str, fn(&str) -> bool>, HashMap<&'static str, fn(&str) -> bool>) {
    ...
    required_fields.insert("hgt", |hgt| {
        if !Regex::new(r"^[0-9]+(cm|in)$").unwrap().is_match(hgt) { return false }; 
    ...
    required_fields.insert("ecl", |ecl| {
        ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].iter().any(|clr| *clr == ecl)
    });
...
pub async fn solve() -> Result<(), Error> {
    ...
    println!("Day 04 Part 1: {:?}",
        parse_passports(common::get_input(2020, 4).await?)
            .iter()
            .filter(|p| is_valid_passport_basic(
                p,
                &required_fields,
                &optional_fields
            ))
    ...
    println!("Day 04 Part 2: {:?}",
        parse_passports(common::get_input(2020, 4).await?)
            .iter()
            .filter(|p| is_valid_passport_advanced(
                p,
                &required_fields,
                &optional_fields
            ))
...
mod tests {
    ...
    async fn test_parse_passport() -> Result<(), Error> {
        let _m = mockito::mock("GET", "/2020/day/4/input")
            .with_status(200)
            .with_body("ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
"
        ...
        assert_eq!(
            ...
            convert_args!(keys=String::from, values=String::from, hashmap!(
                "ecl" => "gry",
                "eyr" => "2020",
                "pid" => "860033327",
                "hcl" => "#fffffd",
...
"
        ...
        let (required_fields, optional_fields) = get_field_rules();
        assert_eq!(is_valid_passport_basic(&parsed_input[0], &required_fields, &optional_fields), true);
        assert_eq!(is_valid_passport_basic(&parsed_input[1], &required_fields, &optional_fields), false);
        assert_eq!(is_valid_passport_basic(&parsed_input[2], &required_fields, &optional_fields), true);
        assert_eq!(is_valid_passport_basic(&parsed_input[3], &required_fields, &optional_fields), false);
        Ok(())
    }
...
"
        ...
        let parsed_input = parse_passports(common::get_input(2020, 4).await?);
        let (required_fields, optional_fields) = get_field_rules();
        assert!(parsed_input.iter().all(|p| !is_valid_passport_advanced(p, &required_fields, &optional_fields)));
        Ok(())
    }
}
